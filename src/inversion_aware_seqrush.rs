use crate::bidirected_graph::{reverse_complement, BiPath, Handle};
use crate::bidirected_ops::BidirectedGraph;
use crate::cigar_analysis::{find_potential_inversion_sites, is_potential_inversion};
use crate::pos::*;
use crate::seqrush::{AlignmentScores, Args, Sequence};
...
impl InversionAwareSeqRush {
    ...
    pub fn build_graph(&mut self, args: &Args) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let alignments = self.find_all_alignments(
            &scores,
        ...
        if args.verbose {
            println!(
                "Found {} alignments (including {} patched inversions)",
                alignments.len(),
                alignments
                    .iter()
                    .filter(|a| a.seq2_is_rc && (a.seq1_start > 0 || a.seq2_start > 0))
                    .count()
            );
        }
        ...
        for i in 0..n {
            for j in i..n {
                if verbose && i != j {
                    println!(
                        "Aligning {} vs {}...",
                        self.sequences[i].id, self.sequences[j].id
                    );
                }
    ...
    fn align_sequences_with_inversions(
        ...
        match self.align_region(
            seq1_idx,
            0,
            self.sequences[seq1_idx].data.len(),
            seq2_idx,
            0,
            self.sequences[seq2_idx].data.len(),
            false,
            scores,
            max_divergence,
        ) {
            Ok((score, cigar)) => {
                if score >= 0 {
                    if verbose {
                        println!(
                            "  Forward-forward alignment {} vs {}: score={}, cigar={}",
                            self.sequences[seq1_idx].id, self.sequences[seq2_idx].id, score, cigar
                        );
                    }
                    ...
                    for gap in &gaps {
                        if is_potential_inversion(gap, min_match_length * 2) {
                            ...
                            if let Ok((inv_score, inv_cigar)) = self.align_region(
                                ...
                                if inv_score >= 0 && inv_score < score / 2 {
                                    // Inversion alignment is good
                                    if verbose {
        ...
        match status {
            AlignmentStatus::Completed => {
                ...
                if let Some(max_div) = max_divergence {
                    let max_allowed_score =
                        scores.max_score_for_divergence(seq1.len().min(seq2.len()), max_div);
    ...
    fn process_alignment(&mut self, alignment: &Alignment, min_match_length: usize, verbose: bool) {
    ...
    fn write_gfa(
        ...
        if verbose {
            println!(
                "Built bidirectional graph: {} nodes, {} edges, {} paths",
                graph.nodes.len(),
                graph.edges.len(),
                graph.paths.len()
            );
