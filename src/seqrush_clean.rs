...
impl SeqRush {
    ...
    pub fn align_and_unite(&self, args: &Args) {
        ...
        println!("Total sequence pairs: {} (sparsification: None)", n * (n - 1));
                            ...
                            if pos1 + k < seq1.data.len() && pos2 + k < seq2_data.len() {
                                if seq1.data[pos1 + k] == seq2_data[pos2 + k] {
                                    ...
                                    if in_match && match_len >= min_match_len {
                                        self.unite_match(seq1, seq2, match_start1, match_start2, match_len, seq2_is_rc);
                                    }
                                    in_match = false;
                                    match_len = 0;
                                }
    ...
    fn unite_match(&self, seq1: &Sequence, seq2: &Sequence, start1: usize, start2: usize, length: usize, seq2_is_rc: bool) {
        static DEBUG_COUNT: std::sync::atomic::AtomicUsize = std::sync::atomic::AtomicUsize::new(0);
        let count = DEBUG_COUNT.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        
        if count < 5 {
            println!("DEBUG unite_match {}: {} -> {} start1={} start2={} len={} rc={}", 
    ...
    fn write_gfa(&self, args: &Args) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (pos, base) in all_positions {
            ...
            union_positions.entry(union_rep)
                .or_default()
                .push((pos, base));
        }
        ...
        if args.verbose {
            ...
            println!("DEBUG: pos 0 rep = {}, pos 12 rep = {}, pos 24 rep = {}", pos0_rep, pos12_rep, pos24_rep);
        ...
        if args.verbose {
            ...
            for (rep, positions) in &union_positions {
                ...
                if positions.len() > 1 {
                    println!("    Positions: {:?}", positions.iter().map(|(p, _)| p).take(5).collect::<Vec<_>>());
                }
            }
        }
...
fn parse_alignment_params(scores_str: &str, max_divergence: Option<f64>) -> AlignmentParams {
    let parts: Vec<i32> = scores_str.split(',')
