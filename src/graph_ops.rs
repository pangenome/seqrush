...
impl Graph {
    ...
    fn compact_nodes_old(&mut self) -> usize {
        ...
        for (iterations, chain) in chains.into_iter().enumerate() {
            if iterations >= max_iterations {
                eprintln!(
                    "WARNING: Compaction stopped after {} iterations",
                    iterations
                );
                break;
            }
    ...
    fn find_linear_chains(&self) -> Vec<Vec<usize>> {
        ...
        for &node_id in self.nodes.keys() {
            ...
            let has_self_loop = self.edges.contains(&Edge {
                from: node_id,
                to: node_id,
            });
            if has_self_loop {
                ...
                if let Some(prevs) = backward_edges.get(&node_id) {
                    if prevs.len() == 1 {
                        ...
                        loop {
                            ...
                            let prev_in =
                                backward_edges.get(&current).map(|v| v.len()).unwrap_or(0);
                            let prev_out =
                                forward_edges.get(&current).map(|v| v.len()).unwrap_or(0);
                ...
                if !visited.contains(&chain_start) {
                    ...
                    while let Some(nexts) = forward_edges.get(&current) {
                        if nexts.len() == 1 {
                            ...
                            let next_out_degree =
                                forward_edges.get(&next).map(|v| v.len()).unwrap_or(0);
    ...
    pub fn topological_sort(&mut self) {
        ...
        let mut problematic_nodes: Vec<(usize, f64)> = node_span_scores
            .iter()
        ...
        for (prob_node, _) in problematic_nodes.iter().take(50) {
            // Limit iterations
            ...
            if !connected_positions.is_empty() {
                ...
                let score: f64 = connected_positions
                    .iter()
    ...
    pub fn reconstruct_path_sequence(
        &self,
        path_name: &str,
        path: &[usize],
        ...
        for &node_id in path {
            match self.nodes.get(&node_id) {
                ...
                None => {
                    return Err(format!(
                        "Path '{}': Node {} not found during reconstruction",
                        path_name, node_id
                    ));
                }
            }
        ...
        if sequence.is_empty() && !path.is_empty() {
            return Err(format!(
                "Path '{}': Reconstruction resulted in empty sequence",
                path_name
            ));
        }
    ...
    pub fn validate_path_structure(&self, verbose: bool) -> Result<(), Vec<String>> {
        ...
        if !orphaned_nodes.is_empty() && verbose {
            println!(
                "Warning: {} nodes are not present in any path",
                orphaned_nodes.len()
            );
        }
    ...
    pub fn verify_edge_traversal(&self, verbose: bool) -> Result<(), Vec<String>> {
        ...
        if verbose && self_loop_traversals > 0 {
            println!(
                "  Found {} self-loop traversals in paths",
                self_loop_traversals
            );
        }
        ...
        if errors.is_empty() {
            if verbose {
                println!(
                    "✓ Edge traversal verification passed: all {} edges are traversed by paths",
                    self.edges.len()
                );
            }
            Ok(())
    ...
    pub fn validate_gfa_format(&self, verbose: bool) -> Result<(), Vec<String>> {
        ...
        for (id, node) in &self.nodes {
            if *id != node.id {
                errors.push(format!(
                    "Node ID mismatch: map key {} != node.id {}",
                    id, node.id
                ));
            }
            if node.sequence.is_empty() {
    ...
    pub fn comprehensive_verify(
        &self,
        original_sequences: Option<&[(String, Vec<u8>)]>,
        verbose: bool,
    ) -> Result<(), Vec<String>> {
            ...
            for (seq_name, seq_data) in original_seqs {
                if let Some((_, path)) = self.paths.iter().find(|(name, _)| name == seq_name) {
                    ...
                    all_errors.push(format!(
                        "Original sequence '{}' not found in paths",
                        seq_name
                    ));
                }
            }
        ...
        if all_errors.is_empty() {
            ...
            if verbose {
                println!(
                    "✗ Comprehensive verification failed with {} errors",
                    all_errors.len()
                );
