use crate::bidirected_graph::{Handle, BiNode, BiEdge};
...
impl BidirectedGraph {
        ...
        for path in &self.paths {
            ...
            for &handle in &path.steps {
                ...
                let node = self.nodes.get(&node_id)
                    .ok_or_else(|| format!("Path {} references non-existent node {}", path.name, node_id))?;
    ...
    fn find_perfect_neighbor_pairs(&self) -> Vec<(Handle, Handle)> {
        ...
        for &node_id in self.nodes.keys() {
            for is_reverse in [false, true] {
                ...
                let outgoing = outbound.get(&handle).map(|set| set.iter().cloned().collect::<Vec<_>>()).unwrap_or_default();
                ...
                if outgoing.len() == 1 {
                    ...
                    let next_incoming = inbound.get(&next_handle).map(|set| set.iter().cloned().collect::<Vec<_>>()).unwrap_or_default();
    ...
    fn build_connections_from_paths(&self) -> (HashMap<Handle, HashSet<Handle>>, HashMap<Handle, HashSet<Handle>>) {
    ...
    fn apply_compaction(&mut self, ordered_groups: &[Vec<Handle>]) -> Result<usize, String> {
        ...
        for group in ordered_groups {
            ...
            for &handle in group {
                ...
                if handle.is_reverse() {
                    merged_sequence.extend_from_slice(&crate::bidirected_graph::reverse_complement(&node.sequence));
                } else {
            ...
            if new_node_id > 4100 && new_node_id < 4200 {
                eprintln!("Creating merged node {} ({} bp) from {} nodes:", 
                    new_node_id, merged_sequence.len(), group.len());
                for &handle in group {
                    if let Some(node) = self.nodes.get(&handle.node_id()) {
                        eprintln!("  - node {} ({} bp, {})", handle.node_id(), 
                            node.sequence.len(),
            ...
            for &handle in group {
                ...
                handle_remapping.insert(reverse_handle, Handle::new(new_handle.node_id(), !new_handle.is_reverse()));
            }
        ...
        for path in &mut self.paths {
            ...
            while i < path.steps.len() {
                ...
                if let Some(&new_handle) = handle_remapping.get(&handle) {
                    ...
                    i += 1;
                    while i < path.steps.len() && handle_remapping.get(&path.steps[i]) == Some(&new_handle) {
                        i += 1;
                    }
                } else {
    ...
    fn validate_paths_match(&self, original_sequences: &HashMap<String, Vec<u8>>) -> Result<(), String> {
        for path in &self.paths {
            let mut current_sequence = Vec::new();
            
            for &handle in &path.steps {
                let node = self.nodes.get(&handle.node_id())
                    .ok_or_else(|| format!("Path {} references non-existent node {}", path.name, handle.node_id()))?;
                
                if handle.is_reverse() {
                    current_sequence.extend_from_slice(&crate::bidirected_graph::reverse_complement(&node.sequence));
                } else {
                    current_sequence.extend_from_slice(&node.sequence);
                }
            }
            
            let original = original_sequences.get(&path.name)
            ...
            if current_sequence != *original {
                ...
                if let Some(pos) = first_diff {
                    ...
                    eprintln!("    Original: {}", String::from_utf8_lossy(&original[pos.saturating_sub(5)..pos.saturating_add(5).min(original.len())]));
                    eprintln!("    Current:  {}", String::from_utf8_lossy(&current_sequence[pos.saturating_sub(5)..pos.saturating_add(5).min(current_sequence.len())]));
                } else if original.len() != current_sequence.len() {
                    eprintln!("  Sequences match up to position {} but have different lengths", min_len);
                    if current_sequence.len() > original.len() {
                        eprintln!("  Extra bases: {}", String::from_utf8_lossy(&current_sequence[original.len()..]));
                    }
                }
