...
impl SeqwishStyleBuilder {
    ...
    pub fn build_from_paf(
        ...
        let alignment_count = self.process_alignments_paf(paf_path, min_match_len, &mut uf, verbose)?;
    ...
    fn process_alignments_paf(
        ...
        for line in reader.lines() {
            ...
            if alignment_count < 3 && _verbose {
                eprintln!("DEBUG: Alignment {} -> {}: {}..{} to {}..{} (strand {})",
                    query_name, target_name, query_start, query_end, target_start, target_end, query_strand);
                    
                // Also debug the offsets
                let query_offset = self.sequences.iter()
                    .position(|(name, _)| name == query_name)
                    .map(|i| self.seq_offsets[i])
                    .unwrap_or(0);
                let target_offset = self.sequences.iter()
                    .position(|(name, _)| name == target_name)
                    .map(|i| self.seq_offsets[i])
                    .unwrap_or(0);
                eprintln!("DEBUG: query_offset={}, target_offset={}", query_offset, target_offset);
            }
    ...
    fn process_cigar_alignment(
        ...
        while i < cigar_bytes.len() {
            ...
            if unite_count == 0 && (op == '=' || op == 'M') {
                eprintln!("DEBUG: CIGAR op '{}' with count {}, min_match_len={}", op, count, min_match_len);
            }
            ...
            match op {
                '=' => {
                    ...
                    if count >= min_match_len {
                        if unite_count == 0 {
                            eprintln!("DEBUG: Inside '=' case, count={}, min_match_len={}", count, min_match_len);
                        }
                        for j in 0..count {
                            ...
                            if unite_count < 5 {
                                eprintln!("DEBUG: Unite #{}: q_pos={} t_pos={}", unite_count, q_pos, t_pos);
                            }
    ...
    fn build_graph_from_union_find(
        ...
        if verbose {
            eprintln!("[seqwish-style] Found {} union-find components", components.len());
            ...
            eprintln!("[seqwish-style]   {} single-position components", single_pos_components);
            eprintln!("[seqwish-style]   {} multi-position components", multi_pos_components);
            
            // Debug: check total positions covered
            let total_positions: usize = components.values().map(|v| v.len()).sum();
            eprintln!("[seqwish-style] Total positions in components: {} (out of {})", 
        ...
        for path in &graph.paths {
            for window in path.steps.windows(2) {
                let from = window[0];
                let to = window[1];
                if from != to { // Don't create self-loops
