...
impl SeqRushBidirectedSimple {
    ...
    fn process_alignments(
        ...
        for i in 0..n {
            for j in i..n {
                ...
                if let Ok((score, cigar)) =
                    self.align_sequences(i, j, false, false, scores, max_divergence)
                {
                    if score >= 0 {
                        if verbose && i != j {
                            println!(
                                "  Aligned {} to {} (forward-forward): score={}",
                ...
                if i != j {
                    if let Ok((score, cigar)) =
                        self.align_sequences(i, j, false, true, scores, max_divergence)
                    {
                        if score >= 0 {
                            if verbose {
                                println!(
                                    "  Aligned {} to {} (forward-reverse): score={}",
                                    self.sequences[i].id, self.sequences[j].id, score
                                );
                            }
                            self.process_alignment(
    ...
    fn process_match_run(
        ...
        if verbose {
            println!(
                "    Processing match run: seq1[{}..{}] <-> seq2[{}..{}] (len={})",
                start1,
                start1 + length,
                start2,
                start2 + length,
    ...
    fn build_bidirected_graph(
        ...
        for seq in &self.sequences {
            ...
            for i in 0..seq.data.len() {
                ...
                if verbose && i < 3 {
                    println!(
                        "    Graph building: seq {} pos {} -> uf_idx {} -> union {}",
                ...
                let node_id = match union_to_node.get(&union) {
                    ...
                    None => {
                        ...
                        if verbose && i < 3 {
                            println!(
                                "    Creating node {} for seq {} pos {} (union {})",
                                id, seq.id, i, union
                            );
                        }
                        id
