...
pub fn bidirected_unchop(graph: &HashGraph, verbose: bool) -> Result<HashGraph, String> {
    if verbose {
        eprintln!("[unchop] Starting bidirected unchop with {} nodes", graph.node_count());
    }
    ...
    if verbose {
        ...
        eprintln!("[unchop] Handles with single successor: {}/{}", single_out, total_handles);
        eprintln!("[unchop] Handles with single predecessor: {}/{}", single_in, total_handles);
    }
    ...
    for handle in graph.handles() {
        for &start_handle in &[handle, handle.flip()] {
            ...
            if in_deg == 1 && out_deg == 1 {
                ...
                let mut current = start_handle;
                loop {
                    let Some(&next) = successors.get(&current) else { break };
    ...
    if verbose {
        ...
        eprintln!("[unchop] Total handles in chains: {}", total_handles_in_chains);
    ...
    for edge in graph.edges() {
        ...
        if from_new == to_new {
            ...
            let chain = chains.iter().find(|c| c.iter().any(|h| h.id() == from_handle.id()));
