...
fn main() {
    ...
    graph2
        .extend_path(path_id2, Handle::forward(node_a2))
        .unwrap();
    graph2
        .extend_path(path_id2, Handle::forward(node_b2))
        .unwrap();
    graph2
        .extend_path(path_id2, Handle::forward(node_a2))
        .unwrap(); // Visit A again
    graph2
        .extend_path(path_id2, Handle::forward(node_c2))
        .unwrap();
    graph2
        .extend_path(path_id2, Handle::forward(node_b2))
        .unwrap(); // Visit B again
    graph2
        .extend_path(path_id2, Handle::forward(node_c2))
        .unwrap(); // Visit C again
    ...
    let nodes: Vec<_> = (0..7)
        .map(|i| graph3.add_node(vec![b'A' + i as u8]))
        .collect();
    ...
    for &node_idx in &path_nodes {
        graph3
            .extend_path(path_id3, Handle::forward(nodes[node_idx]))
            .unwrap();
    }
    ...
    graph4
        .extend_path(path_id4, Handle::forward(node_a4))
        .unwrap();
    graph4
        .extend_path(path_id4, Handle::forward(node_a4))
        .unwrap(); // Self-loop
    graph4
        .extend_path(path_id4, Handle::forward(node_b4))
        .unwrap();
