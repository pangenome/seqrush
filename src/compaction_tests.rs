...
mod compaction_tests {
    ...
    fn test_partial_chain() {
        ...
        graph
            .paths
            .push(("path1".to_string(), vec![1, 2, 3, 4, 5, 6]));
        graph
            .paths
            .push(("path2".to_string(), vec![7, 2, 3, 4, 5, 6]));
    ...
    fn test_preserve_sequences() {
        ...
        assert_eq!(
            merged_node.sequence,
            vec![b'A', b'T', b'C', b'G', b'A', b'T', b'C', b'G']
        );
    }
    ...
    fn test_path_verification() {
        ...
        let reconstructed = graph
            .reconstruct_path_sequence("path1", &[1, 2, 3, 4])
            .unwrap();
        assert_eq!(reconstructed, vec![b'A', b'B', b'C', b'D']);

        // Test comprehensive verification
        let original_sequences = vec![("path1".to_string(), vec![b'A', b'B', b'C', b'D'])];
        assert!(graph
            .comprehensive_verify(Some(&original_sequences), false)
            .is_ok());
    }
    ...
    fn test_path_verification_after_compaction() {
        ...
        let original_sequences = vec![("path1".to_string(), original_sequence.clone())];
        ...
        let reconstructed = graph
            .reconstruct_path_sequence("path1", &graph.paths[0].1)
            .unwrap();
    ...
    fn test_path_structure_validation() {
        ...
        graph
            .paths
            .push(("excessive_duplicate".to_string(), excessive_duplicates));
