    ...
    fn process_alignments(
        ...
        let results: Vec<_> = pairs
            ...
            .flat_map(|&(i, j)| {
                ...
                if let Ok(result) = Self::align_sequences_static(
                    &sequences[i],
                    &sequences[j],
                    false,
                    false,
        ...
        for (i, j, _seq1_is_rc, seq2_is_rc, cigar, score) in results {
            if verbose && i != j {
                let orientation = if seq2_is_rc {
                    "forward-reverse"
                } else {
                    "forward-forward"
                };
                println!(
                    "  Processing alignment {} to {} ({}): score={}, cigar={}",
            ...
            AlignmentStatus::Completed => {
                ...
                if let Some(max_div) = max_divergence {
                    let max_allowed_score =
                        scores.max_score_for_divergence(pattern.len().min(text.len()), max_div);
                ...
                match ch {
                    'M' | '=' => {
                        ...
                        if match_run_len >= min_match_length {
                            if verbose {
                                println!(
                                    "    Processing match run of length {} at seq1[{}] seq2[{}]",
                            ...
                            for j in 0..match_run_len {
                                ...
                                let pos2_oriented = if seq2_is_rc {
                                    ...
                                    let rev_offset = seq2.offset
                                        + (seq2.data.len() - 1 - (match_run_start2 + j));
                                ...
                                if verbose && j < 2 {
                                    println!(
                                        "      Unite: pos1[{}]={}{} <-> pos2[{}]={}{}",
                                        offset(pos1_fwd),
                                        seq1.data[match_run_start1 + j] as char,
                                        orientation_char(pos1_fwd),
                                        offset(pos2_oriented),
                                        seq2.data[match_run_start2 + j] as char,
                                        orientation_char(pos2_oriented)
                                    );
                                }
