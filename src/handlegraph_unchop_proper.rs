...
use handlegraph::pathhandlegraph::embedded_paths::{MutableGraphPaths, GraphPathNames, IntoPathIds, GraphPathsRef, GraphPaths};
...
pub fn proper_unchop(graph: &HashGraph, verbose: bool) -> Result<HashGraph, String> {
    if verbose {
        eprintln!("[unchop] Starting proper unchop with {} nodes", graph.node_count());
    }
    ...
    for handle in graph.handles() {
        ...
        for &node_handle in &[handle, handle.flip()] {
            ...
            if path_consistent && !nodes_in_pairs.contains(&node_handle.id()) && !nodes_in_pairs.contains(&successor.id()) {
                ...
                if verbose && mergeable_pairs.len() < 10 {
                    eprintln!("[unchop] Can merge {} -> {}", 
                             format!("{:?}{}", node_handle.id(), if node_handle.is_reverse() { "-" } else { "+" }),
                             format!("{:?}{}", successor.id(), if successor.is_reverse() { "-" } else { "+" }));
                }
    ...
    if verbose {
        eprintln!("[unchop] Found {} node pairs to merge", mergeable_pairs.len());
        ...
        eprintln!("[unchop] Nodes with single successor: {}", single_successor_nodes);
        eprintln!("[unchop] Nodes with single predecessor: {}", single_predecessor_nodes);
    }
