    ...
    for line in gfa_content.lines() {
        if line.starts_with('S') {
            ...
            if parts.len() > 2 && !parts[2].is_empty() {
                let steps: Vec<(usize, bool)> = parts[2]
                    .split(',')
                    .map(|s| {
                        let id = s
                            .trim_end_matches('+')
...
fn test_base_case_empty() {
    ...
    let content_lines: Vec<&str> = gfa_content
        .lines()
    ...
    assert_eq!(
        content_lines.len(),
        0,
        "Empty sequence should produce empty graph"
    );
}

#[test]
fn test_base_case_single_base() {
    let sequences = vec![("single", "A")];
...
fn test_inductive_sequence_extension() {
    ...
    for ext in extensions {
        ...
        let sequences = vec![("base", base_seq), ("extended", &extended)];
        ...
        for (node_id, is_rev) in &paths[0].1 {
            ...
            if *is_rev {
                reconstructed_base
                    .push_str(&String::from_utf8(reverse_complement(node_seq.as_bytes())).unwrap());
            } else {
...
fn test_transitivity_of_matches() {
    ...
    let sequences = vec![("A", "ATCGATCG"), ("B", "ATCGATCG"), ("C", "ATCGATCG")];
