...
fn test_complex_rearrangement() {
    ...
    let path1_nodes: Vec<&str> = p_lines[0].split('\t').nth(2).unwrap()
        .split(',')
        .map(|n| n.trim_end_matches('+').trim_end_matches('-'))
        .collect();
    let path2_nodes: Vec<&str> = p_lines[1].split('\t').nth(2).unwrap()
        .split(',')
        .map(|n| n.trim_end_matches('+').trim_end_matches('-'))
        .collect();
...
fn test_self_inverse_palindrome() {
    ...
    assert!(gfa_content.contains("VN:Z:1.0"), "Should have valid GFA header");
}