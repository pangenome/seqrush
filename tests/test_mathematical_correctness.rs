...
fn test_forward_reverse_unification() {
    ...
    for i in 0..4 {
        ...
        assert_eq!(rep_fwd, rep_rev, 
            "Forward and reverse orientations of position {} should have same representative", i);
    }
}
...
fn test_transitive_closure() {
    ...
    seqrush.union_find.unite_matching_region(0, 4, 0, 0, 4, false, 4);
    
    // Unite seq2[0] with seq3[0]
    seqrush.union_find.unite_matching_region(4, 8, 0, 0, 4, false, 4);
    
    // Check transitive closure: seq1[0] should be united with seq3[0]
    let pos1 = make_pos(0, false);
    let pos3 = make_pos(8, false);
    
    assert!(seqrush.union_find.same(pos1, pos3),
...
fn test_single_component_per_position() {
    ...
    seqrush.union_find.unite_matching_region(0, 8, 0, 0, 8, false, 8);
    ...
    assert_eq!(representatives.len(), 8,
        "Expected 8 components, got {}", representatives.len());
}
...
fn test_reverse_complement_alignment() {
    ...
    assert!(seqrush.union_find.same(seq1_3_rev, seq2_0_fwd),
    ...
    assert!(seqrush.union_find.same(seq1_0_rev, seq2_3_fwd),
...
fn test_identical_sequences_produce_minimal_components() {
    ...
    seqrush.union_find.unite_matching_region(0, 8, 0, 0, 8, false, 8);
    seqrush.union_find.unite_matching_region(0, 16, 0, 0, 8, false, 8);
    seqrush.union_find.unite_matching_region(8, 16, 0, 0, 8, false, 8);
...
fn test_no_false_unifications() {
    ...
    assert!(!seqrush.union_find.same(pos1, pos2),
    ...
    assert!(seqrush.union_find.same(pos1, pos1_rev),
