use seqrush::bidirected_graph::{Handle, BiNode, BiPath, BiEdge};
...
fn create_test_graph() -> BidirectedGraph {
    ...
    // Add nodes
    graph.nodes.insert(1, BiNode { id: 1, sequence: vec![b'A'], rank: None });
    graph.nodes.insert(2, BiNode { id: 2, sequence: vec![b'T'], rank: None });
    graph.nodes.insert(3, BiNode { id: 3, sequence: vec![b'C'], rank: None });
    graph.nodes.insert(4, BiNode { id: 4, sequence: vec![b'G'], rank: None });
...
fn test_complex_graph_compaction() {
    ...
    // Main path edges
    graph.edges.insert(BiEdge { from: Handle::forward(1), to: Handle::forward(2) });
    graph.edges.insert(BiEdge { from: Handle::forward(2), to: Handle::forward(3) });
    graph.edges.insert(BiEdge { from: Handle::forward(3), to: Handle::forward(4) });
    graph.edges.insert(BiEdge { from: Handle::forward(4), to: Handle::forward(5) });
    graph.edges.insert(BiEdge { from: Handle::forward(5), to: Handle::forward(6) });
    graph.edges.insert(BiEdge { from: Handle::forward(6), to: Handle::forward(2) });
