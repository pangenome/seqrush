...
impl SequenceGenerator {
    ...
    fn apply_random_inversions(
        ...
        for _ in 0..count {
            ...
            let inverted =
                String::from_utf8(reverse_complement(result[start..end].as_bytes())).unwrap();
...
fn test_motif_based_variations() {
    ...
    for (motif, density) in motifs {
        ...
        let motif_positions: Vec<usize> = seq.match_indices(motif).map(|(pos, _)| pos).collect();
        ...
        for (i, &pos) in motif_positions.iter().enumerate() {
            if i % 2 == 0 && pos > 10 && pos + motif.len() + 10 < variant.len() {
                ...
                let inverted =
                    String::from_utf8(reverse_complement(variant[start..end].as_bytes())).unwrap();
        ...
        sequences.push((Box::leak(format!("{}_normal", motif).into_boxed_str()), seq));
        sequences.push((
...
fn test_complex_structural_variation_patterns() {
    ...
    let mut sequences = vec![("reference", segments.join(""))];
...
fn test_nested_tandem_inversions() {
    ...
    // Create variations
    let mut sequences = vec![("original", level2.clone())];
