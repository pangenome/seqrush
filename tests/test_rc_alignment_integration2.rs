...
fn test_rc_alignment_creates_shared_nodes() {
    ...
    writeln!(paf_file, "seq1\t12\t0\t12\t+\tseq2_rc\t12\t0\t12\t12\t12\t60\tcg:Z:12=").unwrap();
    // Self-alignments
    writeln!(paf_file, "seq1\t12\t0\t12\t+\tseq1\t12\t0\t12\t12\t12\t60\tcg:Z:12=").unwrap();
    writeln!(paf_file, "seq2_rc\t12\t0\t12\t+\tseq2_rc\t12\t0\t12\t12\t12\t60\tcg:Z:12=").unwrap();
    // RC alignment - this is the key one
    writeln!(paf_file, "seq2_rc\t12\t0\t12\t-\tseq1\t12\t0\t12\t12\t12\t60\tcg:Z:12=").unwrap();
    ...
    assert!(node_count < 24, "Sequences should share nodes, not have 24 separate nodes");
...
fn test_complex_rc_alignment_scenario() {
    ...
    assert_eq!(isolated_paths, 0, "No sequence should be isolated, even RC ones");
    
    // Verify all sequences have paths
    assert!(gfa_content.contains("P\tforward\t"), "Forward sequence should have a path");
    assert!(gfa_content.contains("P\treverse\t"), "Reverse sequence should have a path");
    assert!(gfa_content.contains("P\tpartial\t"), "Partial sequence should have a path");
}
...
fn test_palindromic_sequence_handling() {
    ...
    assert!(output.status.success(), "seqrush should handle palindromic sequences");
    ...
    assert!(node_count <= 16, "Identical palindromes should share all nodes");
}