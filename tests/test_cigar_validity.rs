...
fn test_cigar_length_consistency() {
    ...
    for (seq1, seq2, desc) in test_cases {
        ...
        println!(
            "  Query: consumed {} of {} bases",
            query_consumed,
            seq1.len()
        );
        println!(
            "  Target: consumed {} of {} bases",
            target_consumed,
            seq2.len()
        );
        ...
        assert_eq!(
            query_consumed,
            seq1.len(),
            "CIGAR query consumption mismatch for {}",
            desc
        );
        assert_eq!(
            target_consumed,
            seq2.len(),
            "CIGAR target consumption mismatch for {}",
            desc
        );
    }
}
